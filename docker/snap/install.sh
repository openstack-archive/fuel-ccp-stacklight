...
# Dated Jul 19: Merge pull request #1049 from obourdon/multi-metrics
cd $GOPATH/src/github.com/intelsdi-x/snap
git checkout ${REFSPEC}
make deps all install
cp build/plugin/* /usr/local/bin/
# Just to ensure that default/example tasks workflows can be fullfilled
ln -s /usr/local/bin/snap-plugin-processor-passthru $AUTO_DISCOVERY_PATH
# Just to ensure that default/example tasks workflows can be fullfilled
# but also might be interesting to have for debugging purposes
