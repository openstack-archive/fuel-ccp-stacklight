...
-- get the node IP of the node whose name is specified by the CCP_NODE_NAME
-- environment variable. This is done by querying the Kubernetes API.
local function get_node_ip_address()
    local node_name = os.getenv('CCP_NODE_NAME')
    if not node_name then
        return nil, 'CCP_NODE_NAME environment variable not defined'
    end
...
if not kubelet_stats_host then
    ...
    -- we rely on the CCP_NODE_NAME env var and get the kubelet node IP
