...
--   IP of the node specified in the HINDSIGHT_NODE_NAME environment variable
--   will be used. So either the kubelet_stats_host configuration variable or
--   the HINDSIGHT_NODE_NAME environment variable should be set.
...
-- get the node IP of the node whose name is specified by the HINDSIGHT_NODE_NAME
-- environment variable. This is done by querying the Kubernetes API.
local function get_node_ip_address()
    local node_name = os.getenv('HINDSIGHT_NODE_NAME')
    if not node_name then
        return nil, 'HINDSIGHT_NODE_NAME environment variable not defined'
    end
...
if not kubelet_stats_host then
    ...
    -- we rely on the HINDSIGHT_NODE_NAME env var and get the node IP
...
local function collect_pods_stats(pods, prev_stats, curr_stats)
    ...
    for _, pod in ipairs(pods) do
        ...
        local pod_cpu_usage, pod_memory_usage,
            pod_major_page_faults, pod_page_faults,
            pod_working_set =
            collect_pod_stats(pod, prev_stats[pod_uid], curr_stats[pod_uid])
