    ...
    for _, id in ipairs(self.ids_datastore) do
        ...
        if data then
            ...
            if ns - cbuf_time > self.observation_window * 1e9 then
                ...
                if self.fct == 'avg' then
                    -- FIXME(elemoine) check if there's a more efficient way
                    local values
                    values = data.cbuf:get_range(1)
                    local total = 0
                    for _, value in ipairs(values) do
                        total = total + value
                    end
                    local count = 0
                    values = data.cbuf:get_range(2)
                    for _, value in ipairs(values) do
                        count = count + value
                    end
                    result = total / count
                ...
                elseif self.fct == 'diff' then
                    ...
                    -- FIXME(elemoine) compute does not exist anymore, use
                    -- custom code
