...
function process_message ()
    ...
    --- Most of the OS services should match the "pattern_long" e.g. "nova-api"
    --- But some of them dont, e.g "keystone"
    local pattern_short = read_config("pattern_short")
    local pattern_long = read_config("pattern_long")

    local cont_name = read_message("Fields[ContainerName]")
    local program = string.match(cont_name, pattern_long)
    if program == nil then
        local program = string.match(cont_name, pattern_short)
        local service = program
    else
        local service = string.match(program, '(%a)%-%a')
    end

    --- If program is still nil, it means we fail to match current service
    --- using both patterns, so we set fallback one.
    if program == nil then
        local program = "unknown_program"
        local service = "unknown_service"
    end
